...
- **Export:** File → Export (enabled while a project is open). Location defaults to `imagesFolder` (browse can change it); annotation type is locked; mode can change for that export only. Writes one sidecar per image in `imagesFolder` (empty if no boxes): YOLO `.txt` (`labelid xc yc w h`) or Pascal VOC `.xml`. A progress bar runs during the write; **Exported N files.** then the dialog closes.
