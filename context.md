...
**Flow:**
First click on a row selects it (highlight; used as the class for new Box-tool draws). If a canvas box is selected, that click also writes the box’s `labelid` and redraws. Click the already selected row to rename: inline text field + check. Enter or check → trim name (ignore empty) → `updateProject({ labels })` with the same `id` → refresh list. Escape, click outside, or starting another row/Add cancels without saving. VFSln only; does not write `classes.txt`.
...
- **Detections tab:** lists VFSln detections for the current image (label name by `labelid`); refreshes on frame change. Click a row to select that box; hover trash (or Delete / Backspace when a box is selected) removes that detection only.
