...
- **Box tool:** click-drag on the image creates a box (clamped to the image) using the selected Labels-tab class; stays on Box after create. Selecting Box opens the Labels tab (first label if none selected). While dragging, a hatched placeholder, origin dot, and diagonal follow the cursor. Thin black full-stage crosshair lines pass through the cursor tip (visual only).
